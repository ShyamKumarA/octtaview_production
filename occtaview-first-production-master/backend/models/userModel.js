...
userSchema.methods.calculateLevel1ROI = async function () {
  try {
    ...
    const approvedChildLevel1Count = this.childLevel1.filter(
      (child) => child.userStatus === "approved"
    ).length;
    ...
    if (minMembers > directMemberCount) {
      if (previousPackage === "Bronza") {
        const childLevel1Users = await User.find({
          _id: { $in: this.childLevel1 },
        });
...
userSchema.methods.calculateLevel2ROI = async function () {
  try {
    ...
    if (minMembers > directMemberCount) {
      if (previousPackage === "Bronza") {
        const childLevel2Users = await User.find({
          _id: { $in: this.childLevel2 },
        });
...
userSchema.methods.calculateLevel3ROI = async function () {
  try {
    ...
    if (minMembers > directMemberCount) {
      if (previousPackage === "Bronza") {
        const childLevel3Users = await User.find({
          _id: { $in: this.childLevel3 },
        });
